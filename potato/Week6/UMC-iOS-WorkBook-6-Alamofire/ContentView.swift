...
final class ButtonInfoList {
    ...
    static let buttonList: [ButtonInfo] = [
        .init(title: "GET", action: {
            Task {
                await serviceManager.getUser(name: "감자")
            }
        }),
        .init(title: "POST", action: {
            Task {
                await serviceManager.postUser(user: .init(name: "감자", age: 25, address: "서울시 마포구", height: 180))   
            }
            
        }),
        .init(title: "PATCH", action: {
            Task {
                await serviceManager.patchUser(name: "potato")
            }
        }),
        .init(title: "PUT", action: {
            Task {
                await serviceManager.putUser(user: .init(name: "감자", age: 20, address: "경기도 수원시", height: 190))
            }
        }),
