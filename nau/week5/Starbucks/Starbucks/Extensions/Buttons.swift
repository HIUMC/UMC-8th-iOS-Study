...
struct temperatureToggleBtn: View {
    @Binding var selected: CoffeeTemperature
    let onTempChange: (CoffeeTemperature) -> Void
    let availableTemps: [CoffeeTemperature] // [.hot, .iced] 또는 [.hotonly] 등

    var body: some View {
        if availableTemps.count == 1 {
            // HOT ONLY 또는 ICED ONLY
            Text(availableTemps.first?.rawValue ?? "")
                .font(.mainTextSemibold18)
                .foregroundColor(availableTemps == [.hotonly] ? .red : .blue )
                .frame(height: 40)
                .frame(maxWidth: .infinity)
                .background(
                    RoundedRectangle(cornerRadius: 18)
