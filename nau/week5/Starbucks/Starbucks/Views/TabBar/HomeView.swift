...
struct HomeView: View {
    ...
    @Environment(DetailMenuViewModel.self) var viewModel2
    ...
    private var desertView: some View {
        VStack(alignment: .leading) {
            ...
            ScrollView(.horizontal) {
                LazyHStack {
                    ForEach(viewModel.recommendMenuModel2) { model in
                        circleImageCard(model)
                    }
                }
            }.scrollIndicators(.hidden)
    ...
    private func findDetailMenu(for menu: RecommendMenuModel) -> DetailMenuModel? {
        return viewModel2.detailMenuModel.first { $0.menuName == menu.name }
    }
}
